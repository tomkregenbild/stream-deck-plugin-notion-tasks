...
export class NotionTodayDialAction extends SingletonAction<NotionSettings> {
  ...
  override async onWillAppear(ev: WillAppearEvent<NotionSettings>): Promise<void> {
    ...
    const state: DialContextState = {
      ...
      metricIndex: 0,
      initialMetricSelected: false,
    };
    this.contexts.set(action.id, state);
    logger.debug("onWillAppear", { context: state.id, settings: state.settings });

    const cachedSummary = getNotionTodaySummary();
    logger.trace("summary:initial", { context: state.id, hasInitial: Boolean(cachedSummary) });

    await applyLayoutIfNeeded(state);
    await state.action.setFeedback({ ...INITIAL_FEEDBACK });
    await state.action.setTitle("Loading Notion…");

    const firstSummary = cachedSummary ?? (await this.waitForInitialSummary(state));
    if (!this.contexts.has(state.id)) {
      logger.trace("onWillAppear:aborted", { context: state.id, reason: "context-removed" });
      return;
    }

    if (firstSummary) {
      const metrics = dedupeMetrics(firstSummary.metricsOrder ?? DEFAULT_METRICS_ORDER);
      this.ensureInitialMetricSelection(state, metrics);
      logger.trace("summary:initial:metrics", { context: state.id, metrics });
      const initialMetric = metrics[state.metricIndex] ?? "total";
      const initialTitle = buildDialTitle(firstSummary, initialMetric);
      await state.action.setTitle(initialTitle);
      await this.paint(state, firstSummary);
    } else {
      logger.trace("summary:initial:missing", { context: state.id });
    }

    state.unsubscribe = subscribeToNotionSummary(summary => {
      if (!this.contexts.has(state.id)) return;
      if (!summary) {
        logger.trace("summary:update:pending", { context: state.id });
        return;
      }
      const metrics = dedupeMetrics(summary.metricsOrder ?? DEFAULT_METRICS_ORDER);
      this.ensureInitialMetricSelection(state, metrics);
      logger.trace("summary:update", { context: state.id, total: summary.total, completed: summary.completed, active: summary.active });
  ...
  override async onDidReceiveSettings(ev: DidReceiveSettingsEvent<NotionSettings>): Promise<void> {
    ...
    if (!state) {
      logger.debug("onDidReceiveSettings:missing", { context: action.id });
      return;
    }
    state.settings = ev.payload.settings ?? {};
    logger.debug("settings:update", { context: state.id, settings: state.settings });
    state.metricIndex = 0;
    state.initialMetricSelected = false;
  ...
  override async onDialRotate(ev: DialRotateEvent<NotionSettings>): Promise<void> {
    ...
    if (!state) {
      ...
      logger.debug("dial:rotate:missing", { context: action.id });
    ...
    logger.trace("dial:rotate", { context: state.id, ticks });
    ...
    if (!summary) {
      ...
      logger.debug("dial:rotate:no-summary", { context: state.id });
    ...
    if (metrics.length === 0) {
      ...
      logger.debug("paint:no-metrics", { context: state.id });

      logger.debug("dial:rotate:no-metrics", { context: state.id });
    ...
    logger.trace("dial:rotate:index", { context: state.id, currentIndex: state.metricIndex, nextIndex, metricsCount: metrics.length });
  ...
  private async paint(state: DialContextState, summary?: TaskSummary): Promise<void> {
    ...
    logger.trace("paint:start", { context: state.id, metricIndex: state.metricIndex, summaryProvided: Boolean(summary) });

    const currentSummary = summary ?? getNotionTodaySummary();
    if (!currentSummary) {
      logger.debug("paint:no-summary", { context: state.id });
      return;
    }
    ...
    if (state.metricIndex >= metrics.length) {
      logger.trace("paint:normalize-index", { context: state.id, metricIndex: state.metricIndex, metricsCount: metrics.length, reason: "too-large" });
      state.metricIndex = metrics.length - 1;
    }
    if (state.metricIndex < 0) {

      logger.trace("paint:normalize-index", { context: state.id, metricIndex: state.metricIndex, metricsCount: metrics.length, reason: "negative" });
    ...
    logger.debug("paint:metric", { context: state.id, metric, total: currentSummary.total, completed: currentSummary.completed, active: currentSummary.active });

    const dialTitle = buildDialTitle(currentSummary, metric);
    logger.trace("paint:title", { context: state.id, metric, title: dialTitle });
    ...
    logger.trace("paint:feedback", { context: state.id, metric, feedback });

    await state.action.setFeedback(feedback);
    await state.action.setTitle(dialTitle);
    logger.trace("paint:complete", { context: state.id, metric, title: dialTitle });
    logger.debug("print:state", { context: state.id, state: { ...state, action: undefined, unsubscribe: undefined } });
  }

  private async waitForInitialSummary(state: DialContextState): Promise<TaskSummary | undefined> {
    const existing = getNotionTodaySummary();
    if (existing) {
      return existing;
    }
    ...
    return new Promise(resolve => {
      ...
      const handler = (summary: TaskSummary | undefined) => {
        if (!this.contexts.has(state.id)) {
          unsubscribe?.();
          resolve(undefined);
          return;
      ...
      unsubscribe = subscribeToNotionSummary(handler);
    });
  }

  private ensureInitialMetricSelection(state: DialContextState, metrics: DialMetric[]): void {
    if (state.initialMetricSelected) {
      return;
    }
    if (metrics.length === 0) {
      logger.trace("initial-metric:skip", { context: state.id, reason: "no-metrics" });
      return;
    }
    const activeIndex = metrics.indexOf("active");
    const nextIndex = activeIndex >= 0 ? activeIndex : 0;
    if (state.metricIndex !== nextIndex) {
      const selectedMetric = metrics[nextIndex] ?? "total";
      logger.trace("initial-metric:set", { context: state.id, metric: selectedMetric, index: nextIndex });
      state.metricIndex = nextIndex;
    }
    state.initialMetricSelected = true;
  }
}


function buildDialTitle(summary: TaskSummary, metric: DialMetric): string {
  ...
  switch (metric) {
    ...
    case "active":
      ...
      return `Active: ${summary.active}/${summary.total} (${completion}%)`;

    case "nextMeeting":

      if (summary.nextMeeting?.title) {

        return `Next: ${truncate(summary.nextMeeting.title, 20)}`;
    ...
    case "byPillar": {
      ...
      if (top) {
        ...
        return `Pillar: ${truncate(top.label, 20)}`;
    ...
    case "byProject": {
      ...
      if (top) {
        ...
        return `Project: ${truncate(top.label, 20)}`;
    ...
    default:
      ...
      return `Active: ${summary.active}/${summary.total}`;
